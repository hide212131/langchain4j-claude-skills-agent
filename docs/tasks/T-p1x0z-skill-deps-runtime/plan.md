...
- [x] Phase 4: イメージビルドとタグ付け
  - [x] SkillImageTagGenerator を実装（スキルパスから規約ベースでイメージタグを生成）
  - [x] SkillImageBuilder を実装（Dockerfile をビルドし、生成されたタグを付与）
  - [x] Gradle タスクまたは CLI コマンドで全スキルのイメージを一括ビルドできるようにする
  - [x] 単一スキルのみビルドするオプション（`--skill-path` 指定時）を追加
